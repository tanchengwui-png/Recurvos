...
export function SalesQuotationFormPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (quotation) {
        ...
        setCurrency(defaultCurrencyCode(activeCurrencies, companyList.find((company) => company.id === activeCompanyId)?.homeCurrency));
      }
    }
    void load();
  }, [id]);
    ...
    setConfirmState({
      ...
      action: async () => {
        try {
          ...
          if (id) { await api.put(`/sales/quotations/${id}`, payload); navigate("/sales/quotations"); }
          else { const created = await api.post<{ id: string }>(`/sales/quotations`, payload); openCreatedRecord(navigate, "/sales/quotations", "/sales/quotations", created.id); }
        } catch (submitError) {
  ...
  return (
    <div className="page">
      ...
      <section className="card">
        <div className="card-section-header"><div className="section-header-cluster"><h3 className="section-title">Quotation Items</h3></div><button type="button" className="button button-secondary" onClick={() => setLines((current) => [...current, { ...emptyLine }])}>Add item</button></div>
